...
비인증 상태 (초기 진입): 앱을 처음 실행하면 Login/Register View 상태에 머물며, 회원가입을 완료하고 로그인을 통해 인증에 성공하면 Main Dashboard(메인 일정 화면) 상태로 전이.

일정 관리 상태: 메인 화면에서 '새 일정 등록'을 수행하거나 기존 일정을 '조회'하면 Plan Detail(일정 상세) 상태로 진입하여 데이터를 확인 및 편집 가능.

일정 삭제 및 복귀: 일정 상세 뷰 또는 메인 목록에서 특정 일정을 '삭제'하면, 데이터베이스에서 일정이 파기된 후 다시 기본 Main Dashboard 상태로 복귀.

주변 탐색 상태: 메인 상태에서 기능을 전환하여 Nearby Location View 상태로 진입하면, 검색어를 통해 지도상의 주변 명소를 탐색하고 특정 장소의 상세 정보를 확인 가능.

세션 종료 (로그아웃): 앱을 사용하는 도중 어느 화면에서든 '로그아웃'을 실행하면 활성화된 사용자 세션이 파기되며, 즉시 초기 비인증 상태인 Login View로 되돌아감.
